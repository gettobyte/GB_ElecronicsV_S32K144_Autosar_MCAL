...
#define UART_SW_MAJOR_VERSION_C               1
...
static Std_ReturnType Uart_StartSetBuffer(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize, Uart_DataDirectionType Direction);
...
static Std_ReturnType Uart_StartSyncReceive(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize, uint32 Timeout)
{
    ...
    if (UART_STATUS_OPERATION_ONGOING == Uart_Ipw_GetReceiveStatus(Channel, NULL_PTR))
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
        (void)Det_ReportError((uint16)UART_MODULE_ID,
                              (uint8)0,
...
static Std_ReturnType Uart_StartSyncSend(uint8 CoreId, uint8 Channel, const uint8* Buffer, uint32 BufferSize, uint32 Timeout)
{
    ...
    else
    {
        ...
        {
    #endif /* (UART_MULTICORE_SUPPORT == STD_ON) */
    if (UART_STATUS_OPERATION_ONGOING == Uart_Ipw_GetTransmitStatus(Channel, NULL_PTR))
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
        (void)Det_ReportError((uint16)UART_MODULE_ID,
                              (uint8)0,
        ...
        for (u8ChLoop = 0U; u8ChLoop < UART_CH_MAX_CONFIG; u8ChLoop++)
        {
            ...
            if (Uart_apConfig[CoreId]->Configs[u8ChLoop] != NULL_PTR)
            {
                if (CoreId == Uart_apConfig[CoreId]->Configs[u8ChLoop]->ChannelCoreId)
                {
                    ...
                    /* Deinit channel */
                    Uart_Ipw_Deinit(Uart_apConfig[CoreId]->Configs[u8ChLoop]->UartChannelId);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    TempReturn = Uart_StartSetBuffer(CoreId, Channel, Buffer, BufferSize, Direction);
                }
            }
        }
    }
