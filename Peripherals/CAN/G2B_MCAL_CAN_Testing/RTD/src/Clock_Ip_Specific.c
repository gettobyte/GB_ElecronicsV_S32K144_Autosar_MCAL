...
static boolean Clock_Ip_bFircWasEnabledBeforeMcuInit;                 /* Firc clock was enabled before MCU initialization */
...
void SetSimLpoclksRegister_TrustedCall(Clock_Ip_ClockConfigType const *Config)
{
    ...
    for (Index = 0U; Index < Config->SelectorsCount; Index++)
    {
        ...
        {
            SimLpoValue |= ((uint32)(Clock_Ip_au8SelectorEntryPcsHardwareValue[Config->Selectors[Index].Value]) << SIM_LPOCLKS_LPOCLKSEL_SHIFT);
        }
    }
...
static void Clock_Ip_ClockInitializeObjects(void)
{
    static boolean Clock_Ip_bObjsAreInitialized = FALSE;   /* Clock objects are initialized. */
