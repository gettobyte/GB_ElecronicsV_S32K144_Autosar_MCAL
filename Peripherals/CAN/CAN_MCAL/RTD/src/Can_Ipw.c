...
#define CAN_IPW_ALL_ERR_INT     ((uint32)0x003B0006U)
...
static void Can_Ipw_SetRxMb(const uint8 u8Controller, const Can_HwObjectConfigType * Can_pHwObject)
{
   ...
   (void)FlexCAN_Ip_ConfigRxMb(u8Controller, Can_pHwObject->Can_u8HwBufferIndex, &dataInfo, Can_pHwObject->Can_pHwFilterConfig->Can_u32HwFilterCode);
}
/*==================================================================================================
*                                       GLOBAL FUNCTIONS
==================================================================================================*/
...
static Std_ReturnType Can_Ipw_SetControlerErrorInit(const Can_ControllerConfigType * Can_pControllerConfig)
{
    ...
    FlexCAN_Ip_ClearErrorStatus(Can_pControllerConfig->Can_u8ControllerOffset, CAN_IPW_ALL_ERR_INT);
    if (TRUE == Can_Ipw_abIsInterruptEnabled[Can_pControllerConfig->Can_u8ControllerID])
        ...
        if (((Can_pControllerConfig->HwChannelIpConfig->pFlexcanIpHwConfig->ctrlOptions & FLEXCAN_IP_BUSOFF_RECOVERY_U32) == 0U) && (FLEXCAN_STATUS_SUCCESS == eRetVal))
        {
            /* Try to recover from bus-off if possible */
            eRetVal = FlexCAN_Ip_ManualBusOffRecovery(Can_pControllerConfig->Can_u8ControllerOffset);
        }
        return (Std_ReturnType)eRetVal;
}
...
Std_ReturnType Can_Ipw_SetControllerToStartMode(const Can_ControllerConfigType * Can_pControllerConfig)
{
    ...
    if (FLEXCAN_STATUS_SUCCESS == eRetVal)
    {
        ...
        for (u8HwObjRefIdx = 0U; u8HwObjRefIdx < Can_pControllerConfig->Can_u8HwObjectRefCount; u8HwObjRefIdx++)
        {
            ...
            {
                Can_Ipw_SetRxMb(Can_pControllerConfig->Can_u8ControllerOffset, Can_pHwObject);
            }
...
)
{
    ...
    #else
        DataInfo.enable_brs = Can_pControllerConfig->Can_pBaudrateConfig[0U].Can_bBitRateSwitch;
    #endif
#endif
        DataInfo.is_remote = FALSE;
...
Std_ReturnType Can_Ipw_InitRx(const Can_ControllerConfigType * Can_pController)
{
    ...
    for (u8HwObjRefIdx = 0U; u8HwObjRefIdx < Can_pController->Can_u8HwObjectRefCount; u8HwObjRefIdx++)
    {
        ...
        if (CAN_RX_ENHANCED_FIFO == Can_pHwObject->Can_eReceiveType)
        {
            for (u8FilterIdx=0U; u8FilterIdx < Can_pHwObject->Can_u8HwFilterCount; u8FilterIdx++)
            {
                ...
                RxFifoFiltersEnhanced[u8FilterIdx].filterType = (Flexcan_Ip_EnhancedFilterType)((Can_pHwObject->Can_pHwFilterConfig[u8FilterIdx].Can_u32HwFilterCode & 0xC0000000U) >> 30U);
    ...
    return (FLEXCAN_STATUS_SUCCESS == FlexCAN_Ip_SetStopMode(Can_pController->Can_u8ControllerOffset)) ? ((Std_ReturnType)E_OK) : ((Std_ReturnType)E_NOT_OK);
}
        ...
        do
        {
            if (FLEXCAN_MB_TX_BUSY == ((Can_Ipw_apxStatus[Can_pControllerConfig->Can_u8ControllerID])->mbs[Can_pHwObjectConfig->Can_u8HwBufferIndex + u8ObjIdx].state))
            {
                ...
                if (FLEXCAN_STATUS_SUCCESS == FlexCAN_Ip_GetTransferStatus(Can_pControllerConfig->Can_u8ControllerOffset, Can_pHwObjectConfig->Can_u8HwBufferIndex + u8ObjIdx))
                {
                    ...
                    if (TRUE == Can_pHwObjectConfig->CanTimestampEnable)
                    {
                        uint32 timestamp = Can_Ipw_apxStatus[Can_pControllerConfig->Can_u8ControllerOffset]->mbs[Can_pHwObjectConfig->Can_u8HwBufferIndex + u8ObjIdx].time_stamp;
    ...
    )
    {
        ...
        if ((CAN_HWOBJ_UNMAPPED != u8HwObjectID) && (FALSE == Can_pHwObjectConfig[u8HwObjectID].Can_bHwObjectUsesPolling))
        {
            if (CAN_TX_NORMAL == Can_pHwObjectConfig[u8HwObjectID].Can_eReceiveType)
            {
                ...
                {
                    TimeStamp = Can_Ipw_apxStatus[Can_pControllerConfig->Can_u8ControllerOffset]->mbs[u8MbIdx].time_stamp;
...
Std_ReturnType Can_Ipw_DeactivateIcomConfiguration(const Can_ControllerConfigType * Can_pControllerConfig)
{
    ...
    {
        IsStart = FlexCAN_Ip_GetStartMode(Can_pControllerConfig->Can_u8ControllerOffset);
