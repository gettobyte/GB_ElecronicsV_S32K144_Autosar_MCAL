...
#define FTM_PWM_IP_SW_PATCH_VERSION_C                  1
...
static Ftm_Pwm_Ip_PolarityType Ftm_Pwm_Ip_aPolarity[FTM_PWM_IP_INSTANCE_COUNT][FTM_PWM_IP_CHANNEL_COUNT];
...
static void Ftm_Pwm_Ip_UpdateNotificationState(uint8 Instance, uint8 Channel, Ftm_Ftm_Ip_EdgeNotifType EdgeNotif)
{
    ...
    switch (EdgeNotif)
    {
        case FTM_PWM_IP_BOTH_EDGES:
        {
            Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] |= (Ftm_Pwm_Ip_EdgeInterruptType)(FTM_PWM_IP_TOF_IRQ_EDGE | FTM_PWM_IP_CHF_IRQ_EDGE);

            break;
        }
        case FTM_PWM_IP_RISING_EDGE:
        {
            if (FTM_PWM_IP_POLARITY_HIGH == Ftm_Pwm_Ip_aPolarity[Instance][Channel])
            {
                Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] |= FTM_PWM_IP_TOF_IRQ_EDGE;
            }
            else
            {
                Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] |= FTM_PWM_IP_CHF_IRQ_EDGE;
            }

            break;
        }
        case FTM_PWM_IP_FALLING_EDGE:
        {
            if (FTM_PWM_IP_POLARITY_LOW == Ftm_Pwm_Ip_aPolarity[Instance][Channel])
            {
                Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] |= FTM_PWM_IP_TOF_IRQ_EDGE;
            }
            else
...
static void Ftm_Pwm_Ip_UpdateNotificationStateCombine(uint8 Instance, uint8 Channel, Ftm_Ftm_Ip_EdgeNotifType EdgeNotif)
{
    ...
    switch (EdgeNotif)
    {
        ...
        {
            Ftm_Pwm_Ip_aNotifIrq[Instance][Channel]      |= FTM_PWM_IP_CHF_IRQ_EDGE;
            Ftm_Pwm_Ip_aNotifIrq[Instance][Channel + 1U]   |= FTM_PWM_IP_CHF_IRQ_EDGE;
                                   ...
                                   const Ftm_Pwm_Ip_ChannelConfigType * UserChCfg)
{
    ...
    Ftm_Pwm_Ip_aPolarity[Instance][ChannelId] = UserChCfg->Polarity;
...
void Ftm_Pwm_Ip_DisableNotification(uint8 Instance, uint8 Channel)
{
    ...
    Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] = FTM_PWM_IP_NO_EDGE;
    /* Disable interrupts for given channel */
    Ftm_Pwm_Ip_DisableCmpIrq(Instance, Channel);
                                   ...
                                   Ftm_Ftm_Ip_EdgeNotifType EdgeNotif)
{
    ...
    Ftm_Pwm_Ip_aNotifIrq[Instance][Channel] &= (uint8)(~(uint32)(FTM_PWM_IP_CHF_IRQ_EDGE | FTM_PWM_IP_TOF_IRQ_EDGE));
    ...
    /* If duty cycle is not 0% and 100% */
    if ((0U != Ftm_Pwm_Ip_aDutyCycle[Instance][Channel]) &&
        (Ftm_Pwm_Ip_aPeriod[Instance] != Ftm_Pwm_Ip_aDutyCycle[Instance][Channel]))
    {
