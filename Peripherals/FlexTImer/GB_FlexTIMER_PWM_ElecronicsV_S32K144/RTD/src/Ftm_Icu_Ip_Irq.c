...
#define FTM_ICU_IP_IRQ_SW_PATCH_VERSION_C               1
...
#if ((((defined FTM_ICU_0_ISR_USED) || (defined FTM_ICU_1_ISR_USED)) &&  !defined( FTM_ICU_TOF_DISTINCT_LINE)) || \
...
ISR(FTM_0_ISR)
{
    ...
    {
        if ( ((isrStatus & (uint8)(1U << channelIndex)) != (uint8)0U) && \
             (FTM_CSC_CHIE_MASK == (ftmIcuBase[moduleIndex]->CONTROLS[channelIndex].CSC & FTM_CSC_CHIE_MASK)) )
        {
            Ftm_Icu_Ip_ProcessInterrupt(moduleIndex, channelIndex);
        }
    }
...
{
    /*calling process interrupt function of corressponding pair channel*/
