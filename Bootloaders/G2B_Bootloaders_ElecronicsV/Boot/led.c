...
//  /* store the interval time between LED toggles */
	PORTD->PCR[15] = 0x00000100;  /* Port PTD15: MUX = GPIO */
	PTD->PDDR |= 1<<15;        /* Port PTD15: Data Direction= output */
	PTD->PSOR |= 1<<15;        /* Set Output on port D15 (LED on) */
...
//      PTD->PCOR |= GPIO_PCOR_PTCO(0x00000001);
      PTD-> PCOR |= 1<<15;        /* Clear Output on port E23 (LED off) */
    }
    else
    {
      ledOn = BLT_FALSE;
      /* Turn the LED off. */
//      PTD->PSOR |= GPIO_PSOR_PTSO(0x00000001);
      PTD-> PSOR |= 1<<15;        /* Set Output on port E23 (LED on) */
    }
